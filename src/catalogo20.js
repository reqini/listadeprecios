...
const Catalogo20 = () => {
  ...
  const { mobileColumns, toggleColumns } = useColumnLayout('catalogo20', 2);
  ...
  useEffect(() => {
    const loadBankLogos = async () => {
      try {
        ...
        const logos = await getBankLogosForCatalogo('/catalogo20');
        setBankLogos(logos);
      } catch (error) {
        console.warn('No se pudieron cargar logos de bancos para catalogo20:', error);
        setBankLogos([]);
      }
    };
  ...
  useEffect(() => {
    trackCatalogView("Catálogo", "20 cuotas sin interés");
  ...
  useEffect(() => {
    ...
    if (searchTerm && searchTerm.trim()) {
      trackCatalogSearch("Catálogo 20", searchTerm);
    }

    if (cuotasMap["20 cuotas sin interés"]) {
      const cuotaKey = cuotasMap["20 cuotas sin interés"];
      productosFiltrados = productosFiltrados.filter(
  ...
  const addToCart = (product) => {
    const cuotaKeyCatalogo = cuotasMap["20 cuotas sin interés"];
    ...
    const productWithCuota = {
      ...
      selectedCuotaLabel: product.selectedCuotaLabel || "20 cuotas sin interés",
    };

    addItem(productWithCuota);
    trackAddToCart("Catálogo 20", product);
  };
  ...
  return (
    <>
      <Helmet>
        <title>Catálogo 20 Cuotas - Catálogo</title>
      </Helmet>
      ...
      {!loading && productos.length > 0 && (
        <FeaturedProductsBanner
          ...
          onAddToCart={(prod) => {
            addToCart(prod);
            trackAddToCart("Catálogo 20", prod);
          }}
      ...
      {/* Empty state: sin error de red, pero ningún producto tiene precio a 20 cuotas */}
      {!loading && !error && Object.keys(productosAMostrar).length === 0 && (
        <Box sx={{ textAlign: 'center', py: 8, px: 2 }}>
          <Typography variant="h6" gutterBottom>
            Por ahora no hay productos en 20 cuotas sin interés
          </Typography>
