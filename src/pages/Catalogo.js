...
/**
 * Catálogo único para la "familia A" de planes de cuotas (9, 15, 18, 20, 24)
 * — confirmados mecánicamente idénticos entre sí salvo el plan. catalogo3/6/12
 * (con ruleta) y catalogo10/14 (legacy) NO usan este componente, quedan aparte.
 */
const Catalogo = ({ cuota }) => {
  const config = cuotasConfig[cuota];
  ...
  const { mobileColumns, toggleColumns } = useColumnLayout(config.layoutKey, 2);
  ...
  useEffect(() => {
    const loadBankLogos = async () => {
      try {
        ...
        const logos = await getBankLogosForCatalogo(config.route);
        setBankLogos(logos);
      } catch (error) {
        console.warn(`No se pudieron cargar logos de bancos para ${config.layoutKey}:`, error);
        setBankLogos([]);
      }
    };
  ...
  }, [config.route, config.layoutKey]);

  useEffect(() => {
    trackCatalogView("Catálogo", config.label);
  ...
  useEffect(() => {
    ...
    if (searchTerm && searchTerm.trim()) {
      trackCatalogSearch(config.trackingLabel, searchTerm);
    }

    productosFiltrados = productosFiltrados.filter(
      (producto) => producto[config.cuotaKey] && producto[config.cuotaKey] !== 'NO'
    );

    agruparProductosPorLinea(productosFiltrados);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchTerm, productos, productosOriginales, config.cuotaKey, config.trackingLabel]);

  // Añadir producto al carrito
  const addToCart = (product) => {
    const cuotaValueRaw = product[config.cuotaKey] && product[config.cuotaKey] !== 'NO'
      ? product[config.cuotaKey]
      : null;
    const cuotaValue = cuotaValueRaw ? parsePrice(cuotaValueRaw) : null;

    const productWithCuota = {
      ...product,
      selectedCuotaKey: product.selectedCuotaKey || config.cuotaKey,
      selectedCuotaValue: product.selectedCuotaValue || cuotaValue,
      selectedCuotaLabel: product.selectedCuotaLabel || config.label,
    };

    addItem(productWithCuota);
    trackAddToCart(config.trackingLabel, product);
  };
  ...
  const toggleFavorite = (product) => {
    try {
      ...
      if (isCurrentlyFavorite) {
        ...
        trackToggleFavorite(config.trackingLabel, product, false);
      } else {
        updatedFavorites = [...currentFavorites, product];
        message = `${product.descripcion} ha sido agregado a tus favoritos`;
        setSnackbarSeverity('success');
        trackToggleFavorite(config.trackingLabel, product, true);
      }
  ...
  return (
    <>
      <Helmet>
        <title>Catálogo {config.cuota} Cuotas - Catálogo</title>
      </Helmet>
      ...
      {!loading && productos.length > 0 && (
        <FeaturedProductsBanner
          ...
          onAddToCart={(prod) => {
            addToCart(prod);
            trackAddToCart(config.trackingLabel, prod);
          }}
          onToggleFavorite={(prod) => toggleFavorite(prod)}
          selectedCuota={config.label}
          isContado={false}
          cuotaKey={config.cuotaKey}
          cuotasTexto={config.cuotasTexto}
          bankLogos={bankLogos}
        />
      )}
      ...
      {/* Empty state: sin error de red, pero ningún producto tiene precio para este plan */}
      {!loading && !error && Object.keys(productosAMostrar).length === 0 && (
        <Box sx={{ textAlign: 'center', py: 8, px: 2 }}>
          <Typography variant="h6" gutterBottom>
            Por ahora no hay productos en {config.label}
          </Typography>
