...
const Catalogo18 = () => {
  ...
  const { mobileColumns, toggleColumns } = useColumnLayout('catalogo18', 2);
  ...
  useEffect(() => {
    const loadBankLogos = async () => {
      try {
        ...
        const logos = await getBankLogosForCatalogo('/catalogo18');
        setBankLogos(logos);
      } catch (error) {
        console.warn('No se pudieron cargar logos de bancos para catalogo18:', error);
        setBankLogos([]);
      }
    };
  ...
  useEffect(() => {
    trackCatalogView("Catálogo", "18 cuotas sin interés");
  ...
  useEffect(() => {
    ...
    if (searchTerm && searchTerm.trim()) {
      trackCatalogSearch("Catálogo 18", searchTerm);
    }
  ...
  const addToCart = (product) => {
    const cuotaKeyCatalogo = cuotasMap["18 cuotas sin interés"];
    ...
    const productWithCuota = {
      ...
      selectedCuotaLabel: product.selectedCuotaLabel || "18 cuotas sin interés",
    };

    addItem(productWithCuota);
    trackAddToCart("Catálogo 18", product);
  };
  ...
  return (
    <>
      <Helmet>
        <title>Catálogo 18 Cuotas - Catálogo</title>
      </Helmet>
      ...
      {!loading && productos.length > 0 && (
        <FeaturedProductsBanner
          ...
          onAddToCart={(prod) => {
            addToCart(prod);
            trackAddToCart("Catálogo 18", prod);
          }}
      ...
      {/* Empty state: sin error de red, pero ningún producto tiene precio a 18 cuotas */}
      {!loading && !error && Object.keys(productosAMostrar).length === 0 && (
        <Box sx={{ textAlign: 'center', py: 8, px: 2 }}>
          <Typography variant="h6" gutterBottom>
            Por ahora no hay productos en 18 cuotas sin interés
          </Typography>
