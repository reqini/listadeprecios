...
const Catalogo24 = () => {
  ...
  const { mobileColumns, toggleColumns } = useColumnLayout('catalogo24', 2);
  ...
  useEffect(() => {
    const loadBankLogos = async () => {
      try {
        ...
        const logos = await getBankLogosForCatalogo('/catalogo24');
        setBankLogos(logos);
      } catch (error) {
        console.warn('No se pudieron cargar logos de bancos para catalogo24:', error);
        setBankLogos([]);
      }
    };
  ...
  useEffect(() => {
    trackCatalogView("Catálogo", "24 cuotas sin interés");
  ...
  useEffect(() => {
    ...
    if (searchTerm && searchTerm.trim()) {
      trackCatalogSearch("Catálogo 24", searchTerm);
    }
  ...
  const addToCart = (product) => {
    const cuotaKeyCatalogo = cuotasMap["24 cuotas sin interés"];
  ...
  return (
    <>
      <Helmet>
        <title>Catálogo 24 Cuotas - Catálogo</title>
      </Helmet>
      ...
      {!loading && productos.length > 0 && (
        <FeaturedProductsBanner
          ...
          onAddToCart={(prod) => {
            addToCart(prod);
            trackAddToCart("Catálogo 24", prod);
          }}
      ...
      {/* Empty state: sin error de red, pero ningún producto tiene precio a 24 cuotas */}
      {!loading && !error && Object.keys(productosAMostrar).length === 0 && (
        <Box sx={{ textAlign: 'center', py: 8, px: 2 }}>
          <Typography variant="h6" gutterBottom>
            Por ahora no hay productos en 24 cuotas sin interés
          </Typography>
